...
**Browser confirmation exists but is manual, and edit coverage is partial.** Resolution has been confirmed in a running browser for all four tagged objects, including per-instance isolation for GroundCover — clicking different grass instances returns different `variant`, `x`, `z`, and `yaw` values. One full edit round-trip is also browser-confirmed: `Water.waterLevel` went panel edit → disk write → HMR → correct value on reselection, then restored.

What remains unverified is narrower than resolution. `Water.deepColor`, `Water.shallowColor`, `Terrain.noiseFloor`, and `Terrain.lakeBedLevel` have been exercised only through direct `editSource`/`editSourceFile` calls, never through the browser panel. More importantly, none of the browser confirmation is automated — every claim above rests on a human clicking, and there is no regression check that resolution or the edit loop still works after an engine change. The dogfood project has no test suite. See `stage4-exit-condition.md` for the per-value verification table.
